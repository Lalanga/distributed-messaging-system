...
/**
 * Orchestrates the send/receive pipeline:
 *   send()  → timestamp correction → store locally → Raft log → replicate
 *   getMessages() → read from local store (ordered by reorderer)
 */
public class MessageService {
     ...
     *  1. Correct timestamp skew.
     *  2. Store locally.
     *  3. Submit to Raft log (if leader).
     *  4. Replicate to peers.
     *
     * @return true if the message was accepted and stored
     */
    ...
    /**
     * Retrieve messages for a recipient, draining the reorder buffer first
    ...
    public List<Message> getMessages(String recipientId, int limit) {
        ...
        List<Message> ordered = timeSync.getReorderer()
                .drain(recipientId, timeSync.getSynchronizedTime());
